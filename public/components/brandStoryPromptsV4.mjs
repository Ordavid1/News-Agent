...

{
  ...
  "music_arc": "<MANDATORY when total episode duration ≥ 20s (V4 Phase 11, 2026-05-07) — declares per-MOVEMENT music intents that get crossfaded into a score architecture (Setup → Disturbance → Turn → Sting). Replaces the single-bed approach with a Hollywood arc and is the prestige-bar default for any episode longer than a hook-and-reveal commercial. Format: array of { movement: 'setup'|'disturbance'|'turn'|'sting', intent: string, duration_hint_seconds?: number }. Emit even when V4_MOVEMENT_MUSIC=false — the orchestrator will surface it later when the flag flips. For < 20s spots, emit empty array [] AND populate music_bed_intent (flat bed acceptable for very short hooks).>",
